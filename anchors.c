 ...
 */
static int check_anchors_alignment(void)
{
	struct ref *r;
	struct tag *item;
	...
	if (!commit_list_count(shallow_commits))
		return 0;
...
void update_anchors(void)
{
	...
	if (update_anchors.refs.nr || update_anchors.refs_to_keep.nr)
		anchors_update = 1;
	...
	anchors_update += check_anchors_alignment();
	if (anchors_update)
